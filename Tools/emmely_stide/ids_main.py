...
from Tools.emmely_stide.syscallint_extractor import SyscallIntExtractor
from algorithms.stream_ngram_extractor import StreamNgramExtractor
from decision_engine_stide import Stide
...
if __name__ == '__main__':
    ...
    syscall_int_extractor = SyscallIntExtractor()
    tid_extractor = ThreadIDExtractor()

    FE = [syscall_int_extractor, tid_extractor]

    ngram_extractor = StreamNgramExtractor(feature_list=["sys_int"],
                                           thread_aware=True,
                                           ngram_length=4)
    SFE = [ngram_extractor]
    ...
    data_loader = DataLoader('/home/eschulze/LID-DS-2021 Datensatz/CVE-2017-7529')
    ...
    for recording in training_data:
        for syscall in recording.syscalls():
            ...
            if len(stream_feature_list) > 0:
                Stide.train_on(stream_feature_list)
    Stide.fit()
    ...
    for recording in data_loader.validation_data():
        for syscall in recording.syscalls():
            ...
            stream_feature_list = stream_feature_extractor(feature_dict, SFE)
            if len(stream_feature_list) > 0:
                prediction = Stide.predict(stream_feature_list)
                print(prediction)
